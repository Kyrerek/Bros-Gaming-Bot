...
db_pool = AsyncConnectionPool(conninfo=os.getenv("DATABASE_URL"), min_size=2, max_size=10, open=False)#psycopg.connect(os.getenv("DATABASE_URL"))
...
async def on_guild_join(guild):
    ...
    async with db_pool.connection() as db_con:
        async with db_con.cursor() as db_cur:
            await db_cur.execute("""INSERT INTO servers(server_id, server_name) VALUES
...
async def on_guild_remove(guild):
    ...
    async with db_pool.connection() as db_con:
        async with db_con.cursor() as db_cur:
            ...
            await db_cur.execute("""DELETE FROM servers WHERE server_id=%s""", (server_id,))
