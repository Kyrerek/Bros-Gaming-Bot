        ...
        async with db_pool.connection() as db_con:
            async with db_con.cursor() as db_cur:
                await db_cur.execute(
                    """SELECT server_id, alert_channel_id, game_currency FROM servers"""
                )
        ...
        for s in servers:
            if s[1] is not None:
                ...
                async with db_pool.connection() as db_con:
                    async with db_con.cursor() as db_cur:
                        await db_cur.execute(
                            """SELECT store_id, last_price, name, link FROM games
                ...
                for g in games:
                    ...
                    try:
                        if price < g[1]:
                            fields = [
                                Field(
                                    name="Name", value=game_details.title, inline=True
                                ),
                                Field(
                                    name="New price", value=game_details.price_formatted
                                ),
                                Field(
                                    name="Discount",
                                    value=str(game_details.discount) + "%",
                                ),
                            ]
                            e = eg.custom_embed(
                            ...
                            await channel.send(
                                content=role.mention if role else "", embed=e
                            )
                            async with db_pool.connection() as db_con:
                                async with db_con.cursor() as db_cur:
                                    await db_cur.execute(
                                        """UPDATE games SET last_price = %s
                                                    WHERE store_id=%s""",
                                        (price, g[0]),
                                    )
                        elif price > g[1]:
        ...
        for s in servers:
            if s[1] is not None:
                ...
                async with db_pool.connection() as db_con:
                    async with db_con.cursor() as db_cur:
                        await db_cur.execute(
                            """SELECT store_id, not_out, name, link FROM games
                            WHERE server_index=%s""",
                            (s[0],),
                        )
                        games = await db_cur.fetchall()
                for g in games:
                    ...
                    if g[1] and not game_details.not_out:
                        ...
                        fields = [
                            Field(name="Name", value=game_details.title, inline=True),
