...
class Game:
    ...
    def _get_game_deatils_by_steam(self, id, currency):
        ...
        if response.status_code == 200:
            data = response.json()
            data[str(id)]["data"]["short_description"] = data[str(id)]["data"][
                "short_description"
            ].replace("&quot;", '"')
    ...
    def __init__(self, id: int, currency: str):
        ...
        if data is None:
            raise NameError(
                f"{id} is not an existing Steam app id or there is another error with Steam API"
            )
